...
int iniDiaGrep(char*ygecho,char*yggrep,char* grepres);
        ...
        if (strlen(buf) > 0) {
            ...
            if (echoCheck(parted[0]) && count == 1) {
                ...
                if(echoCheck(parted[0])&&grepCheck(parted[1])){
                    ...
                    greped=parted[1]+5;
                    while (*greped == ' ') greped++;
                    iniDiaGrep(echoed,greped,grepres);
                    endl();
                } 
                ...
                if(echoCheck(parted[0])&&grepCheck(parted[1])&&wcCheck(parted[2])){
                    ...
                    greped=parted[1]+5;
                    while (*greped == ' ') greped++;
                    if(iniDiaGrep(echoed,greped,grepres)){
                        iniDiaWc(grepres);
                    }
                    endl();
                }
...
void printString(char* str) {
    int i=0;
    while(str[i] != '\0'){interrupt(0x10,0xE00|str[i],0,0,0);i++;}
}
...
int iniDiaGrep(char* ygecho, char* yggrep, char* grepres) {
    int i=0,j=0,match=0,k;
    while(ygecho[i]!='\0'){
        j = 0;
        while (ygecho[i + j] == yggrep[j] && yggrep[j] != '\0')j++;
        if(yggrep[j] == '\0'){match = 1;break;}
        i++;
    }

    if(match){
        k=0;
        while (ygecho[k] !='\0'){grepres[k] = ygecho[k];k++;}
        grepres[k]='\0'; 
        printString(grepres);
        endl();
        return 1;
